...
test_client = Client.objects.create(name='Test Client Deep', email='test@deep.com', client_type='business', status='active')
test_project = Project.objects.create(client=test_client, name='Test Project Deep', budget=Decimal('100000.00'), status='in_progress', priority=1, start_date=date(2026,1,1), estimated_end_date=date(2026,12,31))
print(f"  Created: {test_client.name} | Project: {test_project.name} (${test_project.budget:,.2f})")

print("\n[3] TEST 1: Creating first invoice ($20,000)...")
inv1 = Invoice.objects.create(client=test_client, project=test_project, amount=Decimal('20000.00'), invoice_date=date(2026,2,1))
print_balances()
...
test("Pending balance = 20k", balance.pending_balance == Decimal('20000.00'), f"Expected $20,000, got ${balance.pending_balance}")

print("\n[4] TEST 2: Creating payment ($10,000) for invoice...")
pay1 = Payment.objects.create(invoice=inv1, client=test_client, project=test_project, amount=Decimal('10000.00'), payment_method='bank_transfer', payment_date=date(2026,2,15))
...
test("Client pending = 10k", balance.pending_balance == Decimal('10000.00'), f"Expected $10,000, got ${balance.pending_balance}")
test("Client total_paid = 10k", balance.total_paid == Decimal('10000.00'), f"Expected $10,000, got ${balance.total_paid}")

print("\n[5] TEST 3: Creating second payment to complete invoice...")
pay2 = Payment.objects.create(invoice=inv1, client=test_client, project=test_project, amount=Decimal('10000.00'), payment_method='bank_transfer', payment_date=date(2026,3,1))
inv1.refresh_from_db()
balance = ClientBalance.objects.get(client=test_client)
print_balances()
print_ledger(test_client.name)
...
inv2 = Invoice.objects.create(client=test_client, project=test_project, amount=Decimal('15000.00'), invoice_date=date(2026,4,1))
inv2.refresh_from_db()
balance = ClientBalance.objects.get(client=test_client)
print_balances()
print_ledger(test_client.name)
print_invoices(test_client.name)
test("Invoice 2 amount = 15k", inv2.amount == Decimal('15000.00'), f"Expected $15,000, got ${inv2.amount}")
test("Client total invoiced = 35k", balance.total_projects_cost == Decimal('35000.00'), f"Expected $35,000, got ${balance.total_projects_cost}")
test("Client pending = 15k", balance.pending_balance == Decimal('15000.00'), f"Expected $15,000, got ${balance.pending_balance}")

print("\n[7] TEST 5: Editing invoice amount...")
...
print_balances()
print_ledger(test_client.name)
ledger_entry = ClientLedger.objects.filter(invoice=inv2, transaction_type='invoice').first()
test("Invoice amount updated", inv2.amount == Decimal('20000.00'), f"Expected $20,000, got ${inv2.amount}")
test("Ledger debit updated", ledger_entry and ledger_entry.debit == Decimal('20000.00'), f"Expected $20,000, got ${ledger_entry.debit if ledger_entry else 'N/A'}")
...
test("Total invoiced = 20k", balance.total_projects_cost == Decimal('20000.00'), f"Expected $20,000, got ${balance.total_projects_cost}")
test("Pending = 10k", balance.pending_balance == Decimal('10000.00'), f"Expected $10,000, got ${balance.pending_balance}")
